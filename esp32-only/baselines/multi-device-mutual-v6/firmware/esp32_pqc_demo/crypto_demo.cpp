...
#include "mutual_policy.h"
#if __has_include("host_trust.h")
#include "host_trust.h"
#else
constexpr unsigned int HOST_TRUST_COUNT = 0;
static const uint8_t HOST_TRUST_KEYS[1][1312] = {};
#endif
...
uint32_t g_mutualStarted = 0, g_mutualLimit = 10;
...
void handlePipelineCamera() {
  ...
  if (action == 1) {
    ...
    if (!pending_rekey::install(request + 10)) { sendError("PIPELINE_INSTALL_FILED"); return; }
  } else if (action == 4) {
...
void handleDsaSign() {
  ...
  if ((messageLength >= sizeof(AUTH_DOMAIN) &&
      ...
      (messageLength >= sizeof(MUTUAL_DEVICE) &&
       memcmp(g_plaintext, MUTUAL_DEVICE, sizeof(MUTUAL_DEVICE)) == 0)) {
...
void mutualFailure(const char *reason) {
  Serial.printf("[AUTH] rejected reason=%s\n", reason);
...
bool mutualMac(const char *domain, size_t domainSize, uint8_t *output, bool epoch) {
  memcpy(g_plaintext, domain, domainSize);
  memcpy(g_plaintext + domainSize, g_mutualContext, sizeof(g_mutualContext));
  memcpy(g_plaintext + domainSize + sizeof(g_mutualContext), g_kemCiphertext, KEM_CIPHERTEXT_SIZE);
  size_t n = domainSize + sizeof(g_mutualContext) + KEM_CIPHERTEXT_SIZE;
  if (epoch) { writeU32Be(g_plaintext + n, g_sessionEpoch); n += 4; }
  return mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256),
                         g_sharedSecret, 32, g_plaintext, n, output) == 0;
}

void handleMutualBegin() {
  g_hostAuthorized = g_mutualPending = false;
  if (!rotateKemKeypair()) { mutualFailure("KEM_KEY_UNAVAILABLE"); return; }
  size_t length = 0;
  if (!demo_protocol::readFrame(g_ciphertext, 68, length) || length != 68) {
    mutualFailure("BAD_MUTUAL_BEGIN"); return;
  }
  uint8_t digest[32];
  bool found = false;
  for (unsigned int i = 0; i < HOST_TRUST_COUNT; ++i) {
    if (mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), HOST_TRUST_KEYS[i], 1312, digest) == 0 &&
        memcmp(digest, g_ciphertext, 32) == 0) { found = true; g_hostIndex = i; break; }
  }
  if (!found) { mutualFailure("HOST_NOT_TRUSTED"); return; }
  g_mutualLimit = (uint32_t(g_ciphertext[64]) << 24) | (uint32_t(g_ciphertext[65]) << 16) |
                  (uint32_t(g_ciphertext[66]) << 8) | g_ciphertext[67];
  if (!g_mutualLimit || g_mutualLimit > MAX_REKEY_INTERVAL) { mutualFailure("BAD_REKEY_INTERVAL"); return; }
  memcpy(g_mutualContext, g_ciphertext, 32);
  if (mbedtls_md(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), g_dsaPublicKey, 1312, g_mutualContext + 32) != 0) {
    mutualFailure("MUTUAL_HASH_FAILED"); return;
  }
  memcpy(g_mutualContext + 64, g_ciphertext + 32, 32);
  esp_fill_random(g_mutualContext + 96, 32);
  memcpy(g_mutualContext + 128, g_kemPublicKey, KEM_PUBLIC_KEY_SIZE);
  memcpy(g_mutualContext + 1312, g_ciphertext + 64, 4);
  memcpy(g_plaintext, MUTUAL_DEVICE, sizeof(MUTUAL_DEVICE));
  memcpy(g_plaintext + sizeof(MUTUAL_DEVICE), g_mutualContext, sizeof(g_mutualContext));
  size_t signatureLength = 0;
  if (MLDSA44::sign(g_signature, &signatureLength, g_plaintext,
      sizeof(MUTUAL_DEVICE) + sizeof(g_mutualContext), g_dsaSecretKey) != 0 || signatureLength != 2420) {
    mutualFailure("MUTUAL_SIGN_FAILED"); return;
  }
  g_mutualStarted = millis(); g_mutualPending = true;
  const demo_protocol::ResponseFrame parts[] = {{g_mutualContext + 96, 32}, {g_kemPublicKey, 1184}, {g_signature, 2420}};
  if (!demo_protocol::writeResponse("OK\n", parts, 3)) mutualFailure("MUTUAL_TX_FAILED");
}

void handleMutualFinish() {
  if (!g_mutualPending || millis() - g_mutualStarted > 30000) { mutualFailure("MUTUAL_EXPIRED"); return; }
  g_mutualPending = false; // A challenge can be consumed only once, even on failure.
  size_t length = 0;
  if (!demo_protocol::readFrame(g_ciphertext, 3540, length) || length != 3540 || millis() - g_mutualStarted > 30000) {
    mutualFailure("BAD_MUTUAL_FINISH"); return;
  }
  memcpy(g_kemCiphertext, g_ciphertext, 1088);
  memcpy(g_plaintext, MUTUAL_HOST, sizeof(MUTUAL_HOST));
  memcpy(g_plaintext + sizeof(MUTUAL_HOST), g_mutualContext, sizeof(g_mutualContext));
  memcpy(g_plaintext + sizeof(MUTUAL_HOST) + sizeof(g_mutualContext), g_kemCiphertext, 1088);
  if (MLDSA44::verify(g_ciphertext + 1088, 2420, g_plaintext,
      sizeof(MUTUAL_HOST) + sizeof(g_mutualContext) + 1088, HOST_TRUST_KEYS[g_hostIndex]) != 0) {
    mutualFailure("HOST_SIGNATURE_REJECTED"); return;
  }
  const uint32_t started = millis();
  if (PQCLEAN_MLKEM768_CLEAN_crypto_kem_dec(g_sharedSecret, g_kemCiphertext, g_kemSecretKey) != 0) {
    mutualFailure("KEM_DECAP_FAILED"); return;
  }
  uint8_t proof[32];
  if (!mutualMac(MUTUAL_HOST_KEY, sizeof(MUTUAL_HOST_KEY), proof, false)) { mutualFailure("MUTUAL_MAC_FAILED"); return; }
  uint8_t different = 0;
  for (size_t i = 0; i < 32; ++i) different |= proof[i] ^ g_ciphertext[3508 + i];
  if (different || g_sessionEpoch == UINT32_MAX) { mutualFailure("HOST_KEY_PROOF_REJECTED"); return; }
  ++g_sessionEpoch;
  if (!mutualMac(MUTUAL_DEVICE_KEY, sizeof(MUTUAL_DEVICE_KEY), proof, true)) { mutualFailure("MUTUAL_MAC_FAILED"); return; }
  g_rekeyInterval = g_mutualLimit; g_messageCount = 0;
  g_hostAuthorized = g_sessionReady = true;
  Serial.printf("[AUTH] Host verified allowlist_index=%u epoch=%lu\n", g_hostIndex, g_sessionEpoch);
  char status[100];
...
void handleCommand(const char *command) {
  if (demo_transport::wifiMode()) {
    if (strcmp(command, "MUTUAL_BEGIN") == 0) { handleMutualBegin(); return; }
    if (strcmp(command, "MUTUAL_FINISH") == 0) { handleMutualFinish(); return; }
    if (!g_hostAuthorized && !mutual_policy::publicCommand(command)) {
      mutualFailure("HOST_AUTH_REQUIRED"); return;
    }
    // No legacy path may install an unsigned KEM ciphertext on WiFi.
    if (mutual_policy::legacyCommand(command)) {
      mutualFailure("USE_MUTUAL_HANDSHAKE"); return;
    }
  }
...
bool initializeCrypto() {
  ...
  // mDNS allocates internal RAM. Reserve the unchanged 48 KiB worker stack
  ...
  if (demo_transport::wifiMode()) {
    ...
    Serial.println("[PIPELINE] worker reserved stack_bytes=49152 before discovery");
  }
