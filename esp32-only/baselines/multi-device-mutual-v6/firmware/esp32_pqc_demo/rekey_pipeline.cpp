...
Workspace *work = nullptr;  // PSRAM; WiFi reserves this before starting mDNS
...
bool allocate() {
  ...
  if (!work) {
    Serial.printf("[PIPELINE] allocation failed stage=workspace requested=%u\n", static_cast<unsigned int>(sizeof(Workspace)));
    return false;
  }
  ...
  if (xTaskCreate(worker, "pqc-next", 49152, nullptr, 1, &task) != pdPASS) {
    Serial.printf("[PIPELINE] allocation failed stage=task stack_bytes=49152 largest_internal_before=%u\n", static_cast<unsigned int>(largest));
    heap_caps_free(work); work = nullptr; task = nullptr; return false;
  }
  return true;
}
}
bool prepare() { return allocate(); }
