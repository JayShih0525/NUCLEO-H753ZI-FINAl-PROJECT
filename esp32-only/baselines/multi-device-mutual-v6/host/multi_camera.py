...
def main(argv=None):
    ...
    parser.add_argument('--discover', action='store_true', help='discover all configured identities, including disabled entries')
    ...
    if not math.isfinite(args.discovery_timeout) or args.discovery_timeout <= 0:
        parser.error('discovery-timeout must be positive and finite')
    if args.discover and args.device:
        parser.error('Use --discover for all devices, or --device NAME=auto to select devices')
    if args.dry_run and (args.discover or any(value.endswith('=auto') for value in args.device)):
        parser.error('--dry-run does not perform discovery; use python -m host.discover_devices to browse only')
    ...
    try:
        ...
        if args.discover:
            ...
            if not isinstance(config, dict) or not isinstance(config.get('devices'), list):
                raise ValueError('Configuration must contain a devices list')
            for item in config['devices']:
                if not isinstance(item, dict) or not isinstance(item.get('name'), str):
                    raise ValueError('Invalid device entry')
                overrides[item['name']] = 'auto'
        automatic = {name for name, host in overrides.items() if host == 'auto'}
        devices = load_devices(args.config, {name: f'{name}.invalid' if host == 'auto' else host
                                             for name, host in overrides.items()})
        if automatic:
            from host.discover_devices import discover, resolve_devices
            print('Discovering cameras (advertisements are untrusted; DSA verification follows)...', flush=True)
            discovery_started = time.monotonic()
            found = resolve_devices([device for device in devices if device.name in automatic],
                                    discover(args.discovery_timeout))
            print(f'Discovery completed in {time.monotonic() - discovery_started:.3f}s; matched={len(found)}', flush=True)
            by_name = {device.name: device for device in found}
            devices = [by_name.get(device.name, device) for device in devices]
            if len({(device.host.casefold(), device.port) for device in devices}) != len(devices):
                raise ValueError('Duplicate endpoint after discovery')
