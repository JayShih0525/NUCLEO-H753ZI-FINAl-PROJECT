...
class ReceiveObject:
    ...
    running_crc32: int = 0
    recovery_reply_sent: bool = False
...
class WSRPSession:
    ...
    def handle_data(self, frame: Frame) -> None:
        ...
        if frame.seq == obj.next_seq:
            ...
            # 一個 window 完成，或整筆資料已接完，回 cumulative ACK
            if obj.next_seq % obj.window == 0 or obj.next_seq == obj.total_frames:
                self.send_ack(obj)
        ...
        # ------------------------------------------------------------
        # STM32 沒收到 ACK，所以重送已成功接收過的上一個 window
        #
        # 只在重送 window 的最後一個 frame 到達時重新回 ACK，
        # 避免對每個 duplicate frame 都回 ACK，造成 STM32 收到多個舊 ACK。
        # ------------------------------------------------------------
        if frame.seq < obj.next_seq:
            last_frame_of_accepted_window = obj.next_seq - 1

            if frame.seq == last_frame_of_accepted_window:
                self.send_ack(obj)

                log.info(
                    "RE-ACK object=%d next_seq=%d/%d after duplicate window",
                    obj.object_id,
                    obj.next_seq,
                    obj.total_frames,
                )
        ...
        # ------------------------------------------------------------
        # 收到比期待 seq 更後面的資料，表示中間缺 frame
        # 每次看到 gap 都重新回 NACK，不永久 suppress。
        # ------------------------------------------------------------
        self.send_ack(
            obj,
            nack=True,
        ...
        log.warning(
            "NACK object=%d expected_seq=%d got_seq=%d",
            obj.object_id,
            obj.next_seq,
            frame.seq,
        )
