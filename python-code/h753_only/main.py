...
# 每處理幾張照片就重新做一次 ML-KEM handshake（rekey STM32 keypair + 重新
# encapsulate）換一把新的 AES key。設 0 表示整個 session 都不自動換 key，
# 只在程式一開始做一次 handshake。
REKEY_EVERY_N_FRAMES = 20
...
def hex_preview(data, head: int = 16, tail: int = 16) -> str:
    """
    把 bytes 轉成方便在 dashboard 上顯示的 hex 字串。
    短的（例如 32-byte 的 AES key / shared secret）會整串顯示；
    長的（例如 1000+ bytes 的 public key、2000+ bytes 的簽章）
    只顯示開頭跟結尾各 head/tail bytes，中間用 "..." 省略，
    後面附上完整長度，避免畫面塞不下。
    """
    if data is None:
        return "None"
...
def put_text_fit(img, text, x, y, max_width, scale=0.7, thickness=2, color=(235, 235, 235)):
    """
    畫文字，並保證不超過 max_width（像素）。

    做法：用 cv2.getTextSize 量實際寬度，太寬就把 scale 一路調小，
    調到下限還是太寬的話，直接把字串截斷加「...」。
    這是給「內容長度會變動」的欄位用的（hex 預覽、狀態訊息、說明句子），
    不管內容多長，畫出來永遠不會超出 box 邊界。
    """
...
def put_value(img, label, value, x, y, value_x, max_width, scale=0.75, thickness=2):
    """
    label + value 顯示。value 一律用 put_text_fit，max_width 是從
    value_x 開始，到這個欄位允許畫到的最右邊還剩多少像素（呼叫端自己
    用 panel 的右邊界算），保證不會畫出 box 外面。
    """
...
def make_encrypted_status_image(state):
    ...
    # ========================================================
    # Panel 5: ML-KEM (key exchange) - 每個欄位獨立一行，避免橫向擠爆
    # ========================================================
...
def kem_handshake_with_retry(kem: STM32MLKEM, retry: int = 3, force_rekey: bool = False):
    ...
    force_rekey=True 時，第一次嘗試就先叫 STM32 產生新的 keypair
    （週期性換 key 用；平常開機第一次 handshake 不需要，因為韌體開機
    還沒有 key，get_public_key() 自己就會觸發第一次 rekey）。
    """
    last_error = None
...
class SharedFrameState:
    def __init__(self):
        ...
        # Key 材料的 hex 預覽字串（demo/教學展示用，正式產品不該這樣做）
...
def encrypt_decrypt_thread(
    ...
    try:
        while not state.stop:
            ...
            # --------------------------------------------------
            # 週期性 ML-KEM rekey：每 rekey_every_n_frames 張換一次 AES key
            # --------------------------------------------------
            ...
            try:
                ...
                # payload 已經送出去了，那包資料就會卡在 UART buffer 裡
                # 變成下一個指令的雜訊，導致後面全部連環出錯。
                # 對固定長度的 hash 簽章才是正常做法，也順便讓簽章時間
                # 不會隨照片變大而變慢。
                # ----------------------------------------------
                sig_len = 0
                sign_elapsed = 0.0
...
try:
    ...
    # 明確等待 STM32 開機 banner，確認板子真的在 idle、不是卡在上一個
    # session 中斷時的某個指令中間。新版韌體開機會印 "CRYPTO_APP_READY"；
    # 如果收到的是舊版的 "DILITHIUM_APP_READY"，代表板子上燒的還是舊協定
    # 韌體，不能配這支新版 main.py，要先重新燒新版的 crypto_app_refactor。
